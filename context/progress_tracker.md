...
- [ ] **Phase 3: Indexing and Search**
  ...
  - [ ] Add search run logging for searched/found/missing reporting (Feature 09 persistence).
...
- [ ] **Phase 4: Retrieval and Evidence Packets**
  - [x] Implement two-stage query router (rule-based pre-filter + LLM fallback).
...
* [x] Implement Feature 08 read-only query routing and hybrid retrieval: add validated routing configuration, plural-course/phrase-aware backend search compatibility, deterministic metadata search, optional lazy LLM fallback, RRF provenance models, retrieval orchestration, safe CLI/debug/JSON output, and retrieval telemetry without Feature 09 persistence.
* [x] Add Feature 08 routing/retrieval regression coverage: table-driven no-LLM and overlapping-cue routing matrices for every supported query type, invalid/low-confidence/fatal LLM boundaries, fatal backend failures, zero-hit weaknesses, and deterministic RRF tie ordering.
