...
- [ ] **Phase 5: Answering Interface**
  ...
  - [x] Build FastAPI backend with HTML/JS frontend.

- [ ] **Phase 6: Evaluation and Hardening**
  - [x] Create a hand-curated eval set of 15-20 questions covering each query type (start early, run incrementally).
  - [x] Add `notebooks/evaluation_eda.ipynb` when evaluation reports are written.
  ...
  - [x] Measure retrieval quality and citation quality.
  - [x] Add regression tests for ingestion and evidence packet generation.
...
* [x] Normalize one surrounding Markdown JSON fence from planner LLM responses with a strict regex while preserving exact JSON-object/schema validation, with regression coverage.
...
* [x] Implement Feature 11 FastAPI/HTML UI: provider-lazy health/config endpoints, persisted ask/answer/evidence/coverage APIs, structured references, responsive package-owned static assets, real `app serve` CLI wiring, 20-entry two-hour LRU planner sessions with per-session serialization, cancellation-aware pre-commit answer persistence, sanitized HTTP failures, Chrome desktop/mobile QA, wheel asset verification, and 250 passing tests.
* [x] Implement Feature 12 evaluation and hardening: strict UTF-8 fixture eval-set loading, isolated `data/runs/eval/fixture-state` preparation/manifest validation, deterministic retrieval/citation scoring, paired redacted JSON/Markdown reports with trace IDs and p50/p95 timings, explicit real-archive smoke mode, and a read-only pandas/matplotlib evaluation EDA notebook.
* [x] Harden Feature 12 evaluation boundaries: generate fixture state in a guarded temporary sibling and atomically activate only after full identity validation, detect same-count file/chunk/FTS/vector/Chroma drift, require exact courses-root-relative source matching, propagate persisted search-run ids through retrieval/evidence failures, redact complete authorization values (including comma-delimited parameters for unknown schemes), omit raw queries from reports, resolve the EDA notebook root from either the repository root or `notebooks/`, and verify the complete 304-test suite.
* [x] Implement DEC-039 provider-based embedding construction: add the six-profile registry and lazy provider package, direct Gemini and Nebius adapters, canonical alias propagation, shared validation/retry policy, cloud extras and credential-safe configuration, hosted Chroma/SQLite/retrieval/evaluation coverage, documentation updates, and final 326-test verification.
* [x] Add simple opt-in per-batch progress printing to vector embedding sync and enable it for the `index vector` CLI command.
* [x] Exclude `.ipynb_checkpoints` trees at the inventory source boundary and reset generated state for a clean rebuild.
* [x] Remove inline notebook image data URIs before chunking and add regression coverage without changing the existing generated-state lifecycle.
