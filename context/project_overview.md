...
2. A configured LLM produces a validated query plan with intent and candidate courses/indexes.
...
* **LLM and embedding providers**: Multi-provider via LangChain. Production LLM and embedding model values are nullable configuration loaded from environment variables; a configured LLM provider/model and the optional `llm` extra are mandatory when executing `retrieve`. Production vector commands use only the explicit reviewed profile registry: four local Hugging Face profiles plus the hosted direct-Gemini and Nebius Token Factory profiles documented in Feature 07. Providers are inferred from the selected profile, not a separate provider variable. Tests inject deterministic LangChain doubles at model-loader boundaries. Do not hardcode a paid or cloud provider as required.
